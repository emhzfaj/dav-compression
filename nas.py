...
class VideoCompressorApp(ctk.CTk):
    ...
    Includes a performance dashboard and real-time file status.
    """
    def __init__(self):
        super().__init__()
    ...
    def create_widgets(self):
        ...
        ctk.CTkLabel(header_frame, text="VIDEO COMPRESSION DASHBOARD", font=ctk.CTkFont(family="Monument Extended", size=24, weight="bold"), text_color=self.YELLOW).pack()
        ctk.CTkLabel(header_frame, text="beta version 1.1", font=ctk.CTkFont(family="Monument Extended", size=11), text_color=self.GRAY).pack()
        ...
        # Current File Status
        info_frame = ctk.CTkFrame(config_frame, corner_radius=8, fg_color=self.YELLOW)
        info_frame.grid(row=0, column=1, sticky="nsew", padx=(10, 10))
        info_frame.grid_columnconfigure(0, weight=1)

        ctk.CTkLabel(info_frame, text="CURRENT FILE STATUS", font=ctk.CTkFont(family="Monument Extended", size=14, weight="bold"), text_color=self.BLACK).grid(row=0, column=0, padx=20, pady=(15,10), sticky="w")
        
        self.input_size_label = ctk.CTkLabel(info_frame, text="--- MB", font=ctk.CTkFont(family="Monument Extended", size=24, weight="bold"), text_color=self.BLACK)
        self.input_size_label.grid(row=1, column=0, padx=20, pady=5, sticky="w")
        ctk.CTkLabel(info_frame, text="Input Size", text_color=self.DARK_GRAY, font=ctk.CTkFont(family="Monument Extended")).grid(row=2, column=0, padx=20, sticky="w")
        
        self.output_size_label = ctk.CTkLabel(info_frame, text="--- MB", font=ctk.CTkFont(family="Monument Extended", size=24, weight="bold"), text_color=self.BLACK)
        self.output_size_label.grid(row=3, column=0, padx=20, pady=(15, 5), sticky="w")
        ctk.CTkLabel(info_frame, text="Current Output Size", text_color=self.DARK_GRAY, font=ctk.CTkFont(family="Monument Extended")).grid(row=4, column=0, padx=20, sticky="w")

        self.eta_label = ctk.CTkLabel(info_frame, text="--:--", font=ctk.CTkFont(family="Monument Extended", size=24, weight="bold"), text_color=self.BLACK)
        self.eta_label.grid(row=5, column=0, padx=20, pady=(15, 5), sticky="w")
        ctk.CTkLabel(info_frame, text="Est. Time Remaining", text_color=self.DARK_GRAY, font=ctk.CTkFont(family="Monument Extended")).grid(row=6, column=0, padx=20, pady=(0, 15), sticky="w")
        ...
        self.log_area = ctk.CTkTextbox(bottom_frame, corner_radius=8, font=("Courier", 11), fg_color=self.DARK_GRAY, text_color=self.WHITE, border_width=1, border_color=self.YELLOW)
    ...
    def set_ui_state(self, processing):
        ...
        # Only disable directory entries
        ...
        for line in process.stdout:
            ...
            if len(parts) == 2:
                ...
                if key == 'out_time_ms' and duration > 0:
                    ...
                    if percent > 2: # Avoid division by zero and unstable early estimates
